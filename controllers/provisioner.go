...
// 	p.logger.Debugf("Provision event for OB options: %+v", options)

// 	err := p.initializeCreateOrGrant(options)
// 	if err != nil {
// 		return nil, err
// 	}
// 	p.logger.Infof("Provision: creating bucket %q for OBC %q", p.bucketName, options.ObjectBucketClaim.Name)
...
// 	s3svc, err := object.NewS3Agent(p.accessKeyID, p.secretAccessKey, p.getObjectStoreEndpoint(), p.logger.LevelAt(capnslog.DEBUG), p.tlsCert)
// 	if err != nil {
...
// 		return nil, err
// 	}
// 	p.logger.Infof("set user %q bucket max to %d", p.cephUserName, singleBucketQuota)
...
// 	p.logger.Debugf("Grant event for OB options: %+v", options)
...
// 		return nil, err
// 	}
// 	p.logger.Infof("Grant: allowing access to bucket %q for OBC %q", p.bucketName, options.ObjectBucketClaim.Name)

// 	// check and make sure the bucket exists
// 	p.logger.Infof("Checking for existing bucket %q", p.bucketName)
...
// 	s3svc, err := object.NewS3Agent(objectUser.Keys[0].AccessKey, objectUser.Keys[0].SecretKey, p.getObjectStoreEndpoint(), p.logger.LevelAt(capnslog.DEBUG), p.tlsCert)
// 	if err != nil {
// 		p.deleteOBCResourceLogError("")
// 		return nil, err
// 	}
...
// 	p.logger.Infof("PutBucketPolicy output: %v", out)
// 	if err != nil {
// 		p.deleteOBCResourceLogError("")
// 		return nil, err
// 	}
...
func (p Provisioner) Delete(ob *bktv1alpha1.ObjectBucket) error { return nil }

// 	p.logger.Debugf("Delete event for OB: %+v", ob)

// 	err := p.initializeDeleteOrRevoke(ob)
// 	if err != nil {
// 		return err
// 	}
// 	p.logger.Infof("Delete: deleting bucket %q for OB %q", p.bucketName, ob.Name)
...
func (p Provisioner) Revoke(ob *bktv1alpha1.ObjectBucket) error { return nil }

// 	p.logger.Debugf("Revoke event for OB: %+v", ob)

// 	err := p.initializeDeleteOrRevoke(ob)
// 	if err != nil {
// 		return err
// 	}
// 	p.logger.Infof("Revoke: denying access to bucket %q for OB %q", p.bucketName, ob.Name)
...
// 		s3svc, err := object.NewS3Agent(user.Keys[0].AccessKey, user.Keys[0].SecretKey, p.getObjectStoreEndpoint(), p.logger.LevelAt(capnslog.DEBUG), p.tlsCert)
// 		if err != nil {
// 			return err
// 		}
...
// 				policy = nil
// 				p.logger.Errorf("no bucket policy for bucket %q, so no need to drop policy", p.bucketName)

// 			} else {
// 				p.logger.Errorf("error getting policy for bucket %q. %v", p.bucketName, err)
// 				return err
// 			}
// 		}
...
// 				return err
// 			}
// 			p.logger.Infof("principal %q ejected from bucket %q policy", p.cephUserName, p.bucketName)
// 		}
// 	}
...
// 	p.logger.Info("initializing and setting CreateOrGrant services")
...
// 	if err != nil {
// 		p.logger.Errorf("failed to get storage class for OBC %q in namespace %q. %v", obc.Name, obc.Namespace, err)
// 		return err
// 	}
...
// 		p.logger.Warningf("failed to delete OBC resource. %v", err)
// 	}
// }
...
// 	if p.logger.LevelAt(capnslog.DEBUG) {
// 		p.adminOpsClient, err = admin.New(s3endpoint, accessKey, secretKey, object.NewDebugHTTPClient(httpClient, p.logger))
// 		if err != nil {
...
func (p Provisioner) Update(ob *bktv1alpha1.ObjectBucket) error { return nil }

// 	p.logger.Debugf("Update event for OB: %+v", ob)
